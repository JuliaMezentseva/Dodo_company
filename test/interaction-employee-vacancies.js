...
(async () => {
  try {
    ...
    // Отметка "Есть отклик" — есть отклик на v3 (Продуктовый аналитик) по данным myResponses
    console.log("Отметка 'Есть отклик' на вакансии с уже отправленным откликом:", /Продуктовый аналитик[\s\S]{0,400}Есть отклик/.test(body) || /Есть отклик[\s\S]{0,400}Продуктовый аналитик/.test(body) ? "PASS" : "FAIL");
